...
import { createTransaction, EventBus } from '@spendrail/core';
import {
  PolicyEngine,
  blockAbove,
...
import { SpendTracker, SpendAlerts, SpendAnalytics } from '@spendrail/observe';
...
export class SpendRailStack {
  ...
  constructor(config: McpServerConfig) {
    ...
    this.alerts.onAlert((alert) => {
      ...
      if (this.alertLog.length > SpendRailStack.MAX_ALERT_LOG) {
        this.alertLog.splice(0, this.alertLog.length - SpendRailStack.MAX_ALERT_LOG);
      }
