// =============================================================================
// SpendRailWallet — The orchestrator that connects PolicyEngine + WalletAdapter
//
// This is the single entry point for agents. The flow is:
//   Agent → SpendRailWallet.executePayment() → Policy check → Wallet sign
//
// The agent NEVER touches the wallet directly. SpendRailWallet enforces
...
/**
 * SpendRailWallet is the payment authorization and execution orchestrator for AI agents.
 *
 * It combines:
 ...
 *
 * The agent calls `executePayment()` and SpendRailWallet handles everything:
 ...
 * @example
 * ```ts
 * const wallet = new SpendRailWallet({
...
export class SpendRailWallet {
  ...
  private readonly config: Required<
    Pick<SpendRailWalletConfig, 'defaultCurrency' | 'approvalTimeoutMs' | 'preCheckBalance'>
  > & SpendRailWalletConfig;
  ...
  async executePayment(request: PaymentRequest): Promise<PaymentResult> {
    ...
    this.logger?.info(`[SpendRailWallet] Payment request ${tx.id}`, {
      amount: request.amount,
      currency,
      to: request.to,
      agent: agentId,
    ...
    this.logger?.info(`[SpendRailWallet] Policy result: ${policyResult.action}`, {
      txId: tx.id,
    ...
    try {
      ...
      if (this.spendAlerts) {
        this.spendAlerts.evaluate(tx).catch((err) => {
          this.logger?.error('[SpendRailWallet] Alert evaluation failed', err);
        });
      }

      this.logger?.info(`[SpendRailWallet] Payment completed: ${receipt.txHash}`, {
        txId: tx.id,
      ...
      this.logger?.error(`[SpendRailWallet] Payment failed: ${errorMsg}`, {
        txId: tx.id,
      });
  ...
  private async checkBalance(tx: AgentTransaction): Promise<PaymentResult | null> {
    try {
      ...
      if (available < tx.amount) {
        this.logger?.warn('[SpendRailWallet] Insufficient balance', {
          txId: tx.id,
          required: tx.amount,
          available,
      ...
      this.logger?.warn('[SpendRailWallet] Balance pre-check failed, proceeding', {
  ...
  private async handleApproval(
    ...
    if (!this.config.approvalHandler) {
      this.logger?.warn('[SpendRailWallet] No approval handler configured — denying by default');
      return false;
    }
    ...
    if (result === null) {
      this.logger?.warn('[SpendRailWallet] Approval timed out', { txId: tx.id });
      return null;
    }

    this.logger?.info(`[SpendRailWallet] Approval result: ${result ? 'approved' : 'denied'}`, {
      txId: tx.id,
    });
