...
import { blockAbove, requireApprovalAbove, allowAll } from '@spendrail/control';
import { SpendTracker } from '@spendrail/observe';
import { createTransaction } from '@spendrail/core';
import type { AgentId, PolicyId, AgentTransaction } from '@spendrail/core';
import { SpendRailWallet } from '../src/spendrail-wallet.js';
import type {
  WalletAdapter,
  WalletBalance,
  TransactionReceipt,
...
describe('SpendRailWallet', () => {
  ...
  describe('wallet-agnostic design', () => {
    it('works with any WalletAdapter implementation', async () => {
      ...
      const customWallet = new SpendRailWallet(
        {
          policyEngine,
