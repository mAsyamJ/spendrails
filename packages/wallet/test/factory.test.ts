...
import type { AgentTransaction } from '@spendrail/core';
import { createWallet, presets, SpendRailWallet } from '../src/index.js';
import type {
  WalletAdapter,
  WalletBalance,
  TransactionReceipt,
...
describe('createWallet', () => {
  it('returns a SpendRailWallet instance', () => {
    const wallet = createWallet({
      adapter: { type: 'custom', instance: mockAdapter() },
    });

    assert.ok(wallet instanceof SpendRailWallet);
  ...
  it('passes agentId through', async () => {
    ...
    const txs = wallet.spendTracker.getByAgent('my-agent' as import('@spendrail/core').AgentId);
