// =============================================================================
// Event Bus — typed event system for all SpendRail state changes
...
export type SpendRailEventType = SpendRailEvent['type'];

type EventHandler = (event: SpendRailEvent) => void | Promise<void>;
type TypedHandler<T extends SpendRailEventType> = (
  event: Extract<SpendRailEvent, { type: T }>
) => void | Promise<void>;
...
export class EventBus {
   ...
   */
  on<T extends SpendRailEventType>(type: T, handler: TypedHandler<T>): () => void {
   ...
   */
  once<T extends SpendRailEventType>(type: T, handler: TypedHandler<T>): void {
   ...
   */
  emit(event: SpendRailEvent): void {
