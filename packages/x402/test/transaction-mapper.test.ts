...
import type { X402PaymentPayload, X402PaymentRequirements, X402SpendRailConfig } from '../src/types.js';
import type { AgentId } from '@spendrail/core';
...
describe('extractAgent', () => {
  it('uses payer from payload', async () => {
    const config: X402SpendRailConfig = {};
  ...
  it('falls back to defaultAgentId', async () => {
    ...
    const config: X402SpendRailConfig = { defaultAgentId: 'fallback' as AgentId };
