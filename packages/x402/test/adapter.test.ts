...
describe('SpendRailX402Adapter', () => {
  ...
  describe('wrapFacilitatorClient — verify + settle', () => {
    ...
    it('policy deny returns invalid verify', async () => {
      ...
      assert.ok(verifyResult.invalidReason?.includes('SpendRail policy denied'));
    });
  ...
  describe('createExtension', () => {
    it('returns extension with key', () => {
      ...
      assert.equal(ext.key, 'spendrail');
    });
  });
